...
def runCirc(ncycle:int,shots:int):
    # start = time.time()
    qcis = f'{init_random}'+ncycle*f'{ECM(nD,tH,tCZ,tM,tR)}'+f'{EC(tH,tCZ)}{M_Dire}{M_ALL}'
...
if __name__ == '__main__':
    ...
    for ncycle in range(6,7):
        ...
        np.savetxt(os.path.abspath('')+'/google/result/resultCSS17/qubit_ncycle{ncycle+1}shots{shots}tH400pM0.03pCZ0.02pxyz0.01.txt',result,fmt='%d',delimiter='')
    pools.close()
    pools.join()
